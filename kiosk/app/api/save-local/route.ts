...
/**
 * POST { event_name, seq, kind: 'original'|'ai', variant: 'A'|'B', image_base64 }
 * Writes the photo to C:/semeta/event/<folder>/<conventional-name>. A = local print only.
 */
export async function POST(req: NextRequest) {
  const { event_name, seq, kind, variant, image_base64 } = await req.json()
  ...
  if (variant !== 'A' && variant !== 'B') return NextResponse.json({ error: 'variant must be A or B' }, { status: 400 })
  if (kind !== 'original' && kind !== 'ai') return NextResponse.json({ error: 'kind must be original or ai' }, { status: 400 })

  const filename = kind === 'ai'
    ? aiFilename(event_name, seq, variant)
    : originalFilename(event_name, seq, variant)
